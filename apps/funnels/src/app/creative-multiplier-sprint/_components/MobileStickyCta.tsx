...
export function MobileStickyCta() {
  ...
  useEffect(() => {
    ...
    const updateVisibility = () => {
      ...
      const applyTop =
        apply?.getBoundingClientRect().top ?? Number.POSITIVE_INFINITY;
      ...
      <div className="flex items-center gap-3">
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-medium text-white">
            10 variations / 2 business days
          </p>
          <p className="truncate text-xs text-[#8A8F98]">
            No upload. Book if it fits.
          </p>
        </div>
        <a
          href="#apply"
          onClick={() =>
            trackFunnelEvent("cta_clicked", { placement: "mobile_sticky" })
          }
