...
const MiniCard = ({
  ...
  return (
    <div className={`rounded-xl border p-3 ${toneClass}`}>
      ...
      <p className="mt-2 text-sm leading-tight font-medium text-white">
        {value}
      </p>
    </div>
  );
...
const SlideChrome = ({ index }: { slide: Slide; index: number }) => (
  <span className="absolute top-0 right-0 text-xs text-[#62666D] tabular-nums">
...
const SlideTitle = ({
  ...
  return (
    <h3
      className={`leading-[0.98] font-medium tracking-[-0.055em] text-white ${sizeClass}`}
    >
      {children}
    </h3>
  );
...
const SlideBody = ({ children }: { children: ReactNode }) => (
  <p className="text-[0.88rem] leading-6 text-[#AEB6C2] sm:text-base sm:leading-7">
    {children}
  </p>
);

const MobileProofPill = ({
  label,
  value,
  tone = "neutral",
}: {
  ...
  return (
    <div className={`rounded-lg border px-2.5 py-2 ${toneClass}`}>
      <p className="text-[0.58rem] leading-none font-medium tracking-[0.16em] uppercase">
        {label}
      </p>
      <p className="mt-1.5 text-xs leading-tight font-semibold text-white">
        {value}
      </p>
    </div>
  );
      ...
      return (
        <div className="grid h-full gap-3 pr-14 md:grid-cols-[0.9fr_1fr] md:items-center md:gap-5 md:pr-12">
          ...
          <div className="space-y-4">
            ...
            <p className="max-w-md text-sm leading-6 text-[#AEB6C2]">
              {slide.body}
            </p>
            <div className="grid grid-cols-2 gap-2 md:hidden">
              <MobileProofPill
                label="Winner"
                value="still converts"
                tone="good"
              />
          ...
          <div className="hidden rounded-2xl border border-white/10 bg-black/25 p-4 md:block">
            ...
            <div className="my-4 flex items-center gap-3">
              ...
              <span className="rounded-full border border-[#FF6B1A]/25 bg-[#FF6B1A]/10 px-3 py-1 text-[10px] tracking-[0.18em] text-[#ffb28a] uppercase">
                wrapper aging
              </span>
              <div className="h-px flex-1 bg-white/10" />
            </div>
            <div className="rounded-xl border border-[#FF6B1A]/25 bg-[#FF6B1A]/10 p-3">
              <div className="flex items-center justify-between text-[10px] tracking-[0.18em] text-[#ffb28a] uppercase">
                <span>Same face</span>
              ...
              <p className="mt-3 text-lg leading-tight font-semibold text-white">
                The idea is working. The wrapper is wearing out.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid h-full gap-3 pr-14 md:grid-cols-[0.9fr_1fr] md:items-center md:gap-5 md:pr-12">
          ...
          <div className="hidden rounded-2xl border border-white/10 bg-black/25 p-4 md:block">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <span className="text-xs tracking-[0.2em] text-[#8A8F98] uppercase">
                What still works
              </span>
              <span className="text-emerald-300">
                <CheckIcon />
            ...
            <p className="mt-4 text-2xl leading-tight font-semibold text-white">
              Offer + sales argument
            </p>
            <div className="my-5 h-px bg-white/10" />
            <div className="flex items-center justify-between">
              <span className="text-xs tracking-[0.2em] text-[#ffb28a] uppercase">
                What fatigues first
              </span>
              <span className="rounded-full border border-[#FF6B1A]/25 bg-[#FF6B1A]/10 px-2 py-1 text-[10px] tracking-[0.16em] text-[#ffb28a] uppercase">
                replace
              </span>
            </div>
            <p className="mt-4 text-2xl leading-tight font-semibold text-white">
              Face + delivery wrapper
            </p>
          </div>
        </div>
      );
      ...
      return (
        <div className="grid h-full gap-3 pr-14 md:grid-cols-[0.85fr_1fr] md:items-center md:gap-5 md:pr-12">
          ...
          <div className="space-y-3 md:space-y-4">
            <SlideTitle>{slide.title}</SlideTitle>
            <p className="max-w-md text-sm leading-5 text-[#AEB6C2] md:leading-6">
              {slide.body}
            </p>
          </div>
          <div className="rounded-lg border border-[#FF6B1A]/25 bg-[#FF6B1A]/10 px-2.5 py-2 md:hidden">
            <p className="text-[0.58rem] leading-none font-medium tracking-[0.16em] text-[#ffb28a] uppercase">
              Meanwhile
            </p>
            <p className="mt-1.5 text-xs leading-tight font-semibold text-white">
              Account needs tests now.
            </p>
          </div>
          <div className="relative hidden rounded-2xl border border-white/10 bg-white/[0.035] p-4 md:block">
            <div className="absolute top-[1.375rem] right-[1.35rem] left-[1.35rem] hidden h-px bg-gradient-to-r from-white/20 via-[#FF6B1A]/60 to-[#FF6B1A] md:block" />
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              {delaySteps.map(([label, value], itemIndex) => (
                <div key={label} className="relative">
                  <div
                    className={`relative z-10 mb-4 h-3 w-3 rounded-full ${itemIndex === 3 ? "bg-[#FF6B1A]" : "bg-white/25"}`}
                  />
                  <p className="text-[10px] tracking-[0.18em] text-[#8A8F98] uppercase">
                    {label}
                  </p>
                  <p className="mt-2 text-sm leading-tight font-medium text-white">
                    {value}
            ...
            <div className="mt-7 rounded-xl border border-[#FF6B1A]/25 bg-[#FF6B1A]/10 p-3">
              <p className="text-[10px] tracking-[0.18em] text-[#ffb28a] uppercase">
                Meanwhile
              </p>
              <p className="mt-2 text-lg leading-tight font-semibold text-white">
                The account still needs tests now.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid h-full gap-3 pr-14 md:grid-cols-[1fr_0.9fr] md:items-center md:gap-4 md:pr-0">
          ...
          <div className="hidden gap-2 md:grid">
            {["New hook", "New offer", "New proof"].map((item) => (
              <div
                key={item}
                className="rounded-xl border border-red-400/20 bg-red-400/[0.045] p-3 text-sm font-medium text-white"
              >
                ! {item}
              </div>
            ))}
        ...
        <div className="grid h-full gap-3 pr-14 md:grid-cols-[0.9fr_1fr] md:items-center md:gap-5 md:pr-12">
          ...
          <div className="hidden gap-2 md:grid">
            ...
            <div className="flex items-center justify-center">
              <span className="rounded-full border border-white/10 bg-black/30 px-3 py-1 text-xs tracking-[0.18em] text-[#8A8F98] uppercase">
                change one layer
              </span>
            </div>
            <div className="rounded-2xl border border-[#FF6B1A]/25 bg-[#FF6B1A]/10 p-4">
              <p className="text-xs tracking-[0.2em] text-[#ffb28a] uppercase">
                Swap fast
              </p>
              <p className="mt-3 text-2xl leading-tight font-semibold text-white">
                The visible messenger
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid h-full gap-3 pr-14 md:flex md:flex-col md:justify-between md:gap-4 md:pr-0">
          ...
          <div className="grid flex-1 gap-3 md:grid-cols-[0.95fr_1fr] md:items-center md:gap-4">
            ...
            <div className="relative rounded-xl border border-emerald-400/20 bg-emerald-400/[0.06] p-3 md:rounded-2xl md:p-4">
              ...
              <p className="mt-2 text-xl leading-tight font-semibold text-white md:mt-4 md:text-3xl">
                Winning argument
              </p>
              <div className="mt-3 grid grid-cols-2 gap-2 md:mt-5">
                {["Hook", "Offer", "Proof", "CTA"].map((item) => (
                  <div
                    key={item}
                    className="flex items-center gap-2 rounded-lg border border-emerald-400/15 bg-black/20 p-2 text-xs font-medium text-white"
                  >
                    <span className="text-emerald-300">
                      <CheckIcon />
                    </span>
      ...
      return (
        <div className="flex h-full flex-col justify-between gap-3 pr-0 md:grid md:grid-cols-[0.9fr_1fr] md:items-center md:gap-5 md:pr-12">
          ...
          <div className="space-y-3 pr-14 md:space-y-4 md:pr-0">
            <SlideTitle>{slide.title}</SlideTitle>
            <p className="hidden max-w-md text-sm leading-5 text-[#AEB6C2] md:block md:leading-6">
              {slide.body}
            </p>
          </div>
          <div className="-mx-2 rounded-xl border border-white/10 bg-black/25 p-2 md:mx-0 md:rounded-2xl md:p-4">
            <div className="mb-2 hidden items-center justify-between border-b border-white/10 pb-2 md:mb-3 md:flex md:pb-3">
              <p className="text-xs tracking-[0.2em] text-[#8A8F98] uppercase">
                Avatar layer
              </p>
              <p className="text-xs tracking-[0.18em] text-[#ffb28a] uppercase">
                same script
              </p>
            </div>
              ...
              {[
                {
                  ...
                  mobileLabel: "Asian",
                  face: "bg-[#d6aa78]",
                  body: "w-6 bg-[#2563EB]/25 md:w-8",
                },
                {
                  label: "Black creator",
                  mobileLabel: "Black",
                  face: "bg-[#6f3e27]",
                  body: "w-6 bg-[#FF6B1A]/25 md:w-8",
                },
                {
                  label: "Plus-size creator",
                  mobileLabel: "Plus",
                  face: "bg-[#b97852]",
                  body: "w-8 bg-emerald-400/20 md:w-12",
                },
                {
                  label: "Older expert",
                  mobileLabel: "Older",
                  face: "bg-[#c8a07b]",
                  body: "w-6 bg-white/15 md:w-8",
                },
              ].map((avatar) => (
                <div
                  key={avatar.label}
                  className="rounded-lg border border-white/10 bg-white/[0.035] p-1.5 md:rounded-xl md:p-3"
                >
                  <div className="mx-auto flex h-7 w-7 flex-col items-center justify-end overflow-hidden rounded-full border border-white/10 bg-black/25 md:h-14 md:w-14">
                    <div
                      className={`h-3.5 w-3.5 rounded-full md:h-6 md:w-6 ${avatar.face}`}
                    />
                    <div
                      className={`mt-0.5 h-2 rounded-t-full md:mt-1 md:h-5 ${avatar.body}`}
                    />
                  </div>
                  <p className="mt-1 text-center text-[9px] leading-tight font-medium text-white md:mt-2 md:text-xs">
      ...
      return (
        <div className="grid h-full gap-3 pr-14 md:grid-cols-[0.75fr_1fr] md:items-center md:gap-4 md:pr-0">
          <div className="space-y-4">
            <SlideChrome slide={slide} index={index} />
            <SlideTitle size="small">{slide.title}</SlideTitle>
            <p className="max-w-md text-sm leading-6 text-[#AEB6C2] md:hidden">
              {slide.body}
            </p>
          </div>
          <div className="hidden rounded-2xl border border-[#FF6B1A]/25 bg-[#FF6B1A]/[0.075] p-4 text-center md:block">
            <p className="text-5xl leading-none font-semibold text-white">1</p>
            <p className="text-xs tracking-[0.2em] text-[#ffb28a] uppercase">
              winner
            </p>
            <p className="my-2 text-[#8A8F98]">x avatar layer</p>
            <p className="text-5xl leading-none font-semibold text-white">10</p>
            <p className="text-xs tracking-[0.2em] text-[#ffb28a] uppercase">
              variants
            </p>
          </div>
          ...
          <div className="hidden rounded-2xl border border-white/10 bg-black/25 p-4 md:block">
            ...
            <div className="mt-4 grid gap-2">
              {[
                ...
                <div
                  key={item}
                  className="flex items-center justify-between border-b border-white/10 pb-2 text-sm text-[#D0D6E0] last:border-b-0 last:pb-0"
                >
                  <span>{item}</span>
                  <span className="text-emerald-300">
                    <CheckIcon />
            ...
            <div className="rounded-lg border border-[#FF6B1A]/25 bg-[#FF6B1A]/10 px-2.5 py-2 md:hidden">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-0.5 text-[0.58rem] font-medium tracking-[0.16em] text-[#ffb28a] uppercase">
                    finished files
                  </p>
                </div>
                <div className="grid grid-cols-5 gap-1">
                  {Array.from({ length: 10 }).map((_, itemIndex) => (
                    <span
                      key={itemIndex}
                      className="h-5 w-2.5 rounded-sm border border-white/10 bg-white/[0.06]"
                    />
                  ))}
                </div>
              </div>
          ...
          <div className="hidden rounded-2xl border border-white/10 bg-black/25 p-4 md:block">
            <div className="flex items-end justify-between border-b border-white/10 pb-3">
              <div>
                ...
                <p className="mt-1 text-xs tracking-[0.2em] text-[#ffb28a] uppercase">
                  finished videos
                </p>
              </div>
            ...
            <div className="mt-4 grid gap-2">
              {[
                "Vertical files",
                "Same winning argument",
                "Different avatar delivery",
              ].map((item) => (
                <div
                  key={item}
                  className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] p-3 text-sm font-medium text-white"
                >
        ...
        <div className="grid h-full gap-3 pr-14 md:grid-cols-[0.9fr_1fr] md:items-center md:gap-5 md:pr-12">
          ...
          <div className="hidden rounded-2xl border border-white/10 bg-black/25 p-4 md:block">
            <div className="border-b border-white/10 pb-4">
              <p className="text-6xl leading-none font-semibold text-white">
                48h
              </p>
              <p className="mt-1 text-xs tracking-[0.2em] text-[#ffb28a] uppercase">
                after intake
            ...
            <div className="mt-4 grid grid-cols-3 gap-2">
              {speedSteps.map(([label, value]) => (
                <MiniCard
                  ...
                  label={label}
                  value={value}
                  tone={label === "Day 2" ? "hot" : "neutral"}
                />
              ))}
            </div>
            <p className="mt-4 text-sm leading-6 text-[#AEB6C2]">
              So your buyer gets fresh tests before the existing winner fully
              burns out.
            </p>
          </div>
        </div>
          ...
          <div className="hidden rounded-2xl border border-white/10 bg-black/25 p-4 md:block">
            <div className="rounded-xl border border-red-400/20 bg-red-400/[0.045] p-3">
              ...
              <p className="mt-2 text-sm font-medium text-white">
                awkward / off-brand / unclear
              </p>
            </div>
            <div className="my-3 flex items-center gap-3">
              <div className="h-px flex-1 bg-white/10" />
              <span className="rounded-full border border-[#FF6B1A]/25 bg-[#FF6B1A]/10 px-3 py-1 text-xs font-semibold tracking-[0.16em] text-[#ffb28a] uppercase">
                QA pass
              </span>
              <div className="h-px flex-1 bg-white/10" />
            </div>
      ...
      return (
        <div className="grid h-full gap-3 pr-14 md:grid-cols-[1fr_0.85fr] md:items-center md:gap-4 md:pr-0">
          ...
          <div className="hidden rounded-2xl border border-emerald-400/20 bg-emerald-400/[0.07] p-4 md:block">
            ...
            <p className="mt-3 text-4xl leading-tight font-semibold text-white">
              Refund.
            </p>
...
export function DslCarousel() {
  ...
  return (
    <section
      className="relative mt-3 overflow-hidden bg-transparent px-4 pt-0 pb-5 select-none md:mt-5 md:px-8 md:pt-0 md:pb-12 lg:mt-2"
      aria-labelledby="dsl-heading"
    >
      <div className="relative mx-auto max-w-5xl">
        <div className="mb-2 flex items-end justify-between gap-4 md:mb-3">
          <h2
            id="dsl-heading"
            className="text-sm leading-tight font-medium text-[#D0D6E0] sm:text-base md:text-lg"
          >
            The sprint, explained fast.
          </h2>
          <p className="hidden text-xs font-medium tracking-[0.22em] text-[#62666D] uppercase sm:block">
            Use edges or swipe
          </p>
        </div>
        ...
        <div className="relative">
          <div
            ...
            className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-3 [-ms-overflow-style:none] [scrollbar-width:none] md:pb-0 [&::-webkit-scrollbar]:hidden"
          ...
          >
            {slides.map((slide, index) => (
              ...
              >
                ...
                <div className="absolute -top-20 -right-20 hidden h-56 w-56 rounded-full bg-[#FF6B1A]/10 blur-3xl sm:block" />
          ...
          <button
            ...
            className="group absolute top-0 right-0 z-10 flex h-[255px] w-16 cursor-pointer items-center justify-end rounded-r-xl bg-gradient-to-l from-transparent to-transparent transition hover:from-[#FF6B1A]/12 focus:outline-none disabled:pointer-events-none disabled:opacity-30 sm:h-[360px] sm:w-20 sm:rounded-r-[24px] md:inset-y-0 md:h-auto md:w-24 md:from-black/10 md:hover:from-[#FF6B1A]/14"
          ...
          >
            ...
            />
            {showTapCue ? (
              <span className="click-cue-pill pointer-events-none absolute top-1/2 -right-4 h-9 w-9 rounded-full border border-[#FF6B1A]/35 bg-[#FF6B1A]/20 shadow-[0_0_30px_rgba(255,107,26,0.35)] md:h-11 md:w-11" />
            ) : null}
            <span className="flex h-9 w-9 translate-x-1/2 items-center justify-center rounded-full border border-[#FF6B1A]/25 bg-black/60 text-white shadow-[0_0_22px_rgba(255,107,26,0.20),0_12px_32px_rgba(0,0,0,0.35)] backdrop-blur transition group-hover:border-[#FF6B1A]/45 group-hover:bg-[#B94100]/85 group-hover:shadow-[0_0_30px_rgba(255,107,26,0.34),0_12px_32px_rgba(0,0,0,0.35)] group-focus-visible:ring-2 group-focus-visible:ring-[#FF6B1A]/60 md:h-11 md:w-11">
              <svg
                aria-hidden="true"
        ...
        <div
          className="mt-2 flex w-full flex-nowrap items-center justify-center gap-1 overflow-hidden px-1 md:mt-3"
