...
Telnyx SMS reminders remain fail-closed until `TELNYX_SMS_REMINDERS_ENABLED=true`, `TELNYX_API_KEY`, `TELNYX_FROM_NUMBER`, and the 90-minute and 5-minute `TELNYX_SMS_REMINDER_*_BODY` templates are configured. SMS templates support the same placeholders as Gmail bodies. Both sender and attendee numbers must resolve to E.164; common spaces and punctuation are stripped from submitted attendee numbers. Use a messaging-enabled Telnyx number/profile with the required registration and only enable sends for contacts who have provided the applicable consent. Telnyx SMS shares the same live Cal verification and stale-reminder safeguards as Gmail.
